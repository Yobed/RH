...
import { CheckCircle2, ArrowRight, Check, Clock, AlertCircle } from "lucide-react";
      ...
      <div>
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800 mb-5 gap-4">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-2xl bg-indigo-600 text-white flex items-center justify-center shadow-md shadow-indigo-500/20 shrink-0">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 mt-0.5">
                {pendingCount > 0 ? `${pendingCount} validation(s) en attente d'action` : "Toutes les tâches sont à jour"}
              </p>
            </div>
          </div>
          <Link
            href="/rappels"
            className="text-xs font-black text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 flex items-center gap-1 hover:underline shrink-0 bg-indigo-50 dark:bg-indigo-950/50 px-3 py-1.5 rounded-xl border border-indigo-200/60"
          >
            <span>Voir tout</span>
