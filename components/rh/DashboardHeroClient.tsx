...
function AnimatedBar({ value, delay = 0 }: { value: number; delay?: number }) {
  return (
    <div className="relative h-2 w-full rounded-full bg-slate-100 dark:bg-slate-800/80 overflow-hidden p-0.5 shadow-inner">
      <motion.div
        className="h-full rounded-full bg-gradient-to-r from-[#1e40af] via-[#059669] to-[#017E84]"
        initial={{ width: 0 }}
        animate={{ width: `${value}%` }}
...
export function DashboardHeroClient({ totalActifs, complianceScore, congesEnAttente, dateLabel }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-[#1e40af] via-[#059669] to-[#017E84]" />
      ...
      <div className="absolute top-1/2 -left-24 h-80 w-80 rounded-full bg-[#059669]/10 blur-3xl pointer-events-none" />
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8 sm:gap-10">
        ...
        <div className="space-y-5 max-w-3xl">
          <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-3">
            ...
            <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider bg-[#059669]/15 text-[#059669] border border-[#059669]/30 shadow-xs backdrop-blur-md">
          ...
          <motion.div variants={fadeUp} className="space-y-2">
            <h1 className="text-3xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-slate-900 dark:text-white leading-[1.1]">
              {greeting}, <span className="bg-gradient-to-r from-[#1e40af] via-[#059669] to-[#017E84] bg-clip-text text-transparent">Direction RH</span>
            </h1>
          ...
          <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-3.5 pt-1">
            ...
            {congesEnAttente > 0 ? (
              <div className="flex items-center gap-3 rounded-2xl border border-amber-300/80 bg-amber-50 dark:border-amber-900/60 dark:bg-amber-950/50 px-4.5 py-2.5 shadow-sm backdrop-blur-md hover:border-[#059669] transition-all group cursor-pointer">
                <span className="text-sm font-bold tabular-nums text-[#059669]">{congesEnAttente}</span>
                <span className="text-xs text-amber-950 dark:text-amber-300 font-bold">Demande(s) de Congé</span>
                <span className="px-1.5 py-0.5 rounded-md bg-[#059669] text-[10px] font-bold text-white uppercase tracking-wider ml-1">À valider</span>
              </div>
            ) : (
        ...
        >
          ...
          <div className="mt-4 flex items-center justify-between pt-2">
            ...
            <button className="text-[11px] font-bold text-slate-400 group-hover:text-[#059669] flex items-center gap-0.5 transition-colors">
