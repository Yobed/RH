...
const COLUMNS: Column[] = [
  ...
  { key: "entretien", label: "Entretiens", bg: "bg-slate-50/70 dark:bg-slate-900/40",   headerBorder: "border-amber-300 dark:border-amber-500/30",   badge: "bg-amber-100 text-amber-900 dark:bg-amber-500/20 dark:text-amber-300", accentColor: "bg-[#059669]" },
...
function CandidateCard({ candidate, onDragStart }: CandidateCardProps) {
  ...
  return (
    <div
      ...
      className="group cursor-grab active:cursor-grabbing rounded-2xl border border-slate-200/80 bg-white shadow-sm hover:shadow-xl hover:border-[#059669]/40 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-[#059669]/40 p-4 transition-all duration-250 select-none hover:-translate-y-1 relative overflow-hidden"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-slate-100 font-bold text-xs text-slate-700 dark:bg-slate-800 dark:text-slate-300 group-hover:bg-[#059669] group-hover:text-white transition-colors">
            {getInitials(candidate.full_name)}
          </div>
          <div className="min-w-0">
            <p className="font-bold text-sm text-slate-900 dark:text-white leading-tight truncate group-hover:text-[#059669] transition-colors">{candidate.full_name}</p>
  ...
  return (
    <div
      className={`flex flex-col min-w-[280px] w-[280px] rounded-3xl border transition-all duration-200 ${
        isDragOver
          ? "border-[#059669] bg-[#059669]/5 ring-2 ring-[#059669]/20 scale-[1.01]"
