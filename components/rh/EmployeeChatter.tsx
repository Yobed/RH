...
const ACTIVITY_COLORS: Record<ActivityEntry["type"], string> = {
  document: "bg-[#ee7f03]/10 text-[#ee7f03]",
  conge: "bg-amber-50 text-amber-600",
  sanction: "bg-rose-50 text-rose-600",
  formation: "bg-[#ee7f03]/10 text-[#ee7f03]",
      ...
      {tab !== "activite" && (
        <div className={`px-4 py-3 border-b ${tab === "note" ? "bg-amber-50/40 border-amber-100" : "border-slate-100"}`}>
          <textarea
            ...
            rows={3}
            className={`w-full resize-none rounded-lg border bg-white px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#ee7f03]/40 transition-all ${
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-md bg-[#ee7f03] hover:bg-[#d67002] disabled:opacity-50 disabled:cursor-not-allowed px-3 py-1.5 text-[12px] font-semibold text-white shadow-sm transition-all"
            >
  ...
  return (
    <button
      ...
      className={`relative flex items-center gap-1.5 px-3 py-2 text-[12px] font-semibold transition-colors ${
        active ? "text-[#ee7f03]" : "text-slate-500 hover:text-slate-800"
      }`}
    >
      <Icon className="h-3.5 w-3.5" weight={active ? "fill" : undefined} />
      {label}
      {active && (
        <span className="absolute -bottom-px left-2 right-2 h-0.5 rounded-full bg-[#ee7f03]" />
      )}
    </button>
  );
}
          ...
          <div
            className={`h-9 w-9 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
              n.kind === "note"
                ...
                : "bg-gradient-to-br from-[#ee7f03] to-[#d67002] text-white"
            }`}
          >
