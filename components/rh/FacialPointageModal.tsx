...
export function FacialPointageModal({ isOpen, onClose, onSuccess }: FacialPointageModalProps) {
  ...
  const pointageOptions = [
    ...
    { id: "pause", label: "Pause Déjeuner", icon: Coffee, color: "text-blue-500 bg-blue-50 dark:bg-blue-950/40 border-blue-200" },
    ...
    { id: "depart", label: "Départ Soir", icon: Moon, color: "text-purple-500 bg-purple-50 dark:bg-purple-950/40 border-purple-200" }
  ];
          ...
          <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/30">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 font-medium">Authentification instantanée sans contact</p>
              </div>
            </div>

            <button
              onClick={onClose}
              className="h-8 w-8 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center text-slate-500 transition-colors"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="relative mx-auto w-full aspect-4/3 rounded-2xl bg-slate-950 overflow-hidden border-2 border-slate-800 flex items-center justify-center shadow-inner">
              ...
              {cameraActive ? (
                ...
                <div className="relative w-full h-full flex flex-col items-center justify-center bg-gradient-to-b from-slate-900 via-slate-950 to-indigo-950/40">
                  <div className="relative h-36 w-36 rounded-full border-2 border-dashed border-indigo-500/60 flex items-center justify-center">
                    <div className="h-28 w-28 rounded-full bg-indigo-500/10 border border-indigo-400/30 flex items-center justify-center animate-pulse">
                      <Camera className="h-12 w-12 text-indigo-400 opacity-80" />
                    </div>
                  </div>
                  {cameraError && (
              ...
              <div className="absolute inset-8 pointer-events-none border-2 border-indigo-400/30 rounded-3xl flex items-center justify-center">
                {/* Corner reticles */}
              ...
              <div className="absolute top-3 left-3 right-3 flex items-center justify-between pointer-events-none">
                <div className="bg-slate-900/80 backdrop-blur-md px-2.5 py-1 rounded-full border border-slate-700/60 flex items-center gap-1.5 text-[11px] font-bold text-white">
                  <span className={`h-2 w-2 rounded-full ${scanning ? "bg-amber-400 animate-ping" : success ? "bg-emerald-400" : "bg-indigo-400 animate-pulse"}`} />
            ...
            {scanning && (
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs font-bold">
                  <span className="text-indigo-600 dark:text-indigo-400">{scanStep}</span>
