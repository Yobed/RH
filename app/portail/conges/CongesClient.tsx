...
const STATUT_STYLES: Record<string, { label: string; tone: string; icon: React.ComponentType<{ className?: string }> }> = {
  en_attente: { label: "En attente", tone: "bg-amber-50 text-amber-700 border-amber-200", icon: Clock },
  soumis: { label: "Soumis", tone: "bg-amber-50 text-amber-700 border-amber-200", icon: Clock },
  approuve: { label: "Approuvé", tone: "bg-emerald-50 text-emerald-700 border-emerald-200", icon: Check },
  refuse: { label: "Refusé", tone: "bg-rose-50 text-rose-700 border-rose-200", icon: XCircle },
};
  ...
  return (
    <div className="space-y-6">
      <header className="pb-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold text-slate-900">Mes congés</h1>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 sm:p-5">
        <div className="grid grid-cols-3 gap-3 sm:gap-6 text-center">
          <div>
            <p className="text-[10px] sm:text-[11px] uppercase tracking-wider text-slate-500 font-medium">Acquis {solde.annee}</p>
            <p className="text-lg sm:text-2xl font-semibold text-slate-900 tabular-nums mt-1">{solde.joursAcquis.toFixed(1)} j</p>
          </div>
          <div>
            <p className="text-[10px] sm:text-[11px] uppercase tracking-wider text-slate-500 font-medium">Pris</p>
            <p className="text-lg sm:text-2xl font-semibold text-slate-900 tabular-nums mt-1">{solde.joursPris.toFixed(1)} j</p>
          </div>
          <div>
            <p className="text-[10px] sm:text-[11px] uppercase tracking-wider text-emerald-700 font-medium">Solde</p>
            <p className="text-lg sm:text-2xl font-semibold text-emerald-700 tabular-nums mt-1">{solde.solde.toFixed(1)} j</p>
          </div>
        </div>
      </section>
      ...
      {conges.length === 0 ? (
        <div className="rounded-lg border border-slate-200 bg-white p-12 text-center">
          <CalendarDays className="h-7 w-7 text-slate-300 mx-auto mb-2.5" />
          <p className="text-sm font-medium text-slate-700">Aucune demande</p>
          <p className="text-xs text-slate-500 mt-1">Faites votre première demande de congé.</p>
        </div>
      ) : (
              ...
              <li key={c.id} className="rounded-lg border border-slate-200 bg-white p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {c.commentaire && (
                      <p className="text-xs text-slate-600 mt-2 italic">{c.commentaire}</p>
                    )}
                    {c.statut === "refuse" && c.refus_motif && (
                      <p className="text-xs text-rose-700 mt-2">Motif refus : {c.refus_motif}</p>
                    )}
                  </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-md max-h-[92vh] flex flex-col">
        <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-slate-200">
          <h2 className="text-base font-semibold text-slate-900">Nouvelle demande de congé</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {nbJours > 0 && (
            <div className={`rounded-md border p-3 text-xs ${tooMany ? "border-rose-200 bg-rose-50/60 text-rose-900" : "border-slate-200 bg-slate-50 text-slate-700"}`}>
              <p>
          ...
          <div>
            <label className="text-xs font-medium text-slate-700 block mb-1">Commentaire</label>
            <textarea
              value={commentaire}
        ...
        <div className="flex gap-2 px-5 py-4 border-t border-slate-200">
          <button
            onClick={onClose}
            className="flex-1 h-9 px-4 rounded-md border border-slate-200 bg-white text-sm font-medium text-slate-700 hover:bg-slate-50"
          >
            Annuler
          </button>
          <button
