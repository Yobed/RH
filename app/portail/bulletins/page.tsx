  ...
  return (
    <div className="space-y-6">
      <header className="pb-4 border-b border-slate-200">
        <h1 className="text-xl sm:text-2xl font-semibold text-slate-900">Mes bulletins de paie</h1>
      ...
      {!bulletins || bulletins.length === 0 ? (
        <div className="rounded-lg border border-slate-200 bg-white p-12 text-center">
          ...
          <p className="text-sm font-medium text-slate-700">Aucun bulletin disponible</p>
          <p className="text-xs text-slate-500 mt-1">Vos bulletins apparaîtront ici dès leur génération.</p>
        </div>
      ) : (
            ...
            {bulletins.map((b) => (
              <li key={b.id}>
                <Link
                  ...
                  className="flex items-center justify-between gap-3 px-4 sm:px-5 py-3 hover:bg-slate-50/60 transition-colors"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="inline-flex h-9 w-9 items-center justify-center rounded-md bg-slate-100 text-slate-500 shrink-0">
                      <Calendar className="h-4 w-4" />
                    </span>
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-slate-900 tabular-nums">{b.periode}</p>
