...
export function QhseClient({
  ...
  async function updateStatutCnps(id: string, statut_cnps: string) {
    setUpdatingId(id);
    try {
      const res = await fetch(`/api/qhse/accidents/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ statut_cnps }),
      });
      if (!res.ok) throw new Error();
      setAccidents((prev) =>
        prev.map((a) => a.id === id ? { ...a, statut_cnps: statut_cnps as Accident["statut_cnps"] } : a)
      );
      toast.success("Statut CNPS mis à jour");
    } catch {
                ...
                <tr key={a.id} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {a.statut_cnps === "non_declare" && (
                      <button
                        onClick={() => updateStatutCnps(a.id, "declare")}
