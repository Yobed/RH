...
const FAQS: Faq[] = [
  {
    ...
    a: (
      <>
        ...
        <p className="mt-2">Pour le <strong>réactiver</strong> : <Link href="/employes" className="text-[#0f766e] underline">Collaborateurs</Link> → onglet <em>Inactifs</em> → ouvrir la fiche → <em>Modifier</em> → champ <strong>Statut = Actif</strong> → Enregistrer.</p>
      </>
    ),
  },
  {
    q: "Comment fonctionne la validation des congés ?",
    a: <p>Le salarié (ou le RH) crée la demande dans <Link href="/conges" className="text-[#0f766e] underline">Absences & Congés</Link>. Elle suit le circuit <strong>demande → validation manager → validation RH</strong>. Les soldes s'acquièrent à 2,2 jours/mois travaillé (Code du Travail ivoirien).</p>,
  },
  {
    q: "Quels sont les rôles et leurs accès ?",
    a: <p><strong>Administrateur</strong> et <strong>Responsable RH</strong> : accès complet + administration. <strong>Chargé RH</strong> / <strong>Manager</strong> : périmètre opérationnel et validations. <strong>Collaborateur</strong> : uniquement le <Link href="/portail" className="text-[#0f766e] underline">Portail salarié</Link>. Les accès fins se règlent dans <Link href="/parametres/permissions" className="text-[#0f766e] underline">Rôles & Permissions</Link>.</p>,
  },
  {
    ...
    <PageShell>
      ...
      <div className="relative max-w-xl">
        ...
        <input
          ...
          className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-sm text-slate-900 shadow-2xs outline-none transition-all focus-visible:border-[#0d9488] focus-visible:ring-2 focus-visible:ring-[#0d9488]/20 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
        />
      </div>
      ...
      {modules.map((m) => {
        ...
        return (
          <section key={m.id} className="space-y-3">
            ...
            <div className="flex items-center gap-2.5 rounded-lg border border-[#0d9488]/15 bg-[#0d9488]/10 px-3 py-2">
              <Icon weight="duotone" className="h-4 w-4 text-[#0f766e] dark:text-[#2dd4bf]" />
              <h2 className="text-[13px] font-bold uppercase tracking-[0.06em] text-[#0f766e] dark:text-[#2dd4bf]">{m.label}</h2>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {m.items.map((it) => (
                <Link
                  key={it.href}
                  href={it.href}
                  className="group flex flex-col rounded-2xl border border-slate-200/70 bg-white p-4 transition-all hover:-translate-y-0.5 hover:border-[#0d9488]/40 hover:shadow-[0_10px_24px_-14px_rgba(13,148,136,0.45)] dark:border-slate-800 dark:bg-slate-900"
                >
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="text-sm font-semibold text-slate-900 dark:text-white">{it.title}</h3>
                    <CaretRight weight="bold" className="h-3.5 w-3.5 shrink-0 text-slate-300 transition-transform group-hover:translate-x-0.5 group-hover:text-[#0f766e]" />
                  </div>
                  <p className="mt-1.5 text-[12.5px] leading-relaxed text-slate-500 dark:text-slate-400">{it.desc}</p>
                  <span className="mt-3 inline-flex items-center gap-1 text-[11px] font-semibold text-[#0f766e] opacity-0 transition-opacity group-hover:opacity-100 dark:text-[#2dd4bf]">
